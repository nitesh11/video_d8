...
class TranscoderAbstractionFactoryFfmpeg extends TranscoderAbstractionFactory implements TranscoderFactoryInterface {
  ...
  public function __construct() {
    $this->transcoder = new PHPVideoToolkit(\Drupal::config('video.settings')->get('video_ffmpeg_path') ?: '/usr/bin/ffmpeg', realpath(file_directory_temp()) . '/');
  ...
  public function setInput(array $file) {
    ...
    if (empty($srcpath)) {
      ...
      $srcpath = Videoutility::createTemporaryLocalCopy($srcuri);
      if (empty($srcpath)) {
  ...
  public function setOutput($output_directory, $output_name, $overwrite_mode = FILE_EXISTS_REPLACE) {
    ...
    $tmpoutput = Videoutility::createTempFile(Videoutility::getExtension($output_name));
  ...
  public function extractFrames($destinationScheme, $format) {
    ...
    $dsturibase = $destinationScheme . '://' . $config->get('video_thumbnail_path') ?: 'videos/thumbnails' . '/' . $fid . '/';
    ...
    $no_of_thumbnails = $config->get('video_thumbnail_count') ?: 5;
    ...
    for ($i = 1; $i <= $no_of_thumbnails; $i++) {
      ...
      if (!file_exists($dsturi)) {
        ...
        $dstpath = Videoutility::createTempFile($format);
        ...
        if (class_exists('ffmpeg_movie')) {
          ...
          $result = Videoutility::imageSave($thumb, $dstpath, $format);
          if (!$result) {
  ...
  public function execute() {
    ...
    $config = \Drupal::config('video.settings');
    $drupaldir = getcwd();
    $tmpdir = Videoutility::createTempDir();
    chmod($tmpdir, 0777);
    chdir($tmpdir);
    ...
    $cmd = $config->get('video_ffmpeg_qtfaststart_path');
    ...
    $cmd = $config->get('video_ffmpeg_flvtool2_path');
  ...
  public function adminSettings() {
    ...
    $form['ffmpeg']['video_thumbnail_path'] = array(
      ...
      '#default_value' => $config->get('video_thumbnail_path') ?: 'videos/thumbnails',
    );
    $form['ffmpeg']['video_thumbnail_count'] = array(
      '#type' => 'textfield',
      '#title' => t('Number of thumbnails'),
      '#description' => t('Number of thumbnails to extract from video.'),
      '#default_value' => $config->get('video_thumbnail_count') ?: 5,
      '#size' => 5,
    );
    ...
    $form['ffmpeg']['helpers']['video_ffmpeg_qtfaststart_path'] = array(
      ...
      '#default_value' => $config->get('video_ffmpeg_qtfaststart_path'),
    ...
    $form['ffmpeg']['helpers']['video_ffmpeg_flvtool2_path'] = array(
      ...
      '#default_value' => $config->get('video_ffmpeg_flvtool2_path'),
