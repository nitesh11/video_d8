...
class Transcoder {
  ...
  public function __construct() {
    $transcoder = \Drupal::config('video_ui.settings')->get('video_convertor') ?: new TranscoderAbstractionFactoryFfmpeg();
  ...
  public function executeConversion(stdClass $video) {
    ...
    $thumbnail_number = intval(\Drupal::config('video.settings')->get('video_thumbnail_count') ?: '5');
    ...
    elseif (empty($presets)) {
      ...
      foreach ($presets as $name => $preset) {
        ...
        $preset['settings']['wxh'] = (\Drupal::config('video.settings')->get('video_use_preset_wxh') ?: FALSE) ? $preset['settings']['wxh'] : $video->dimensions;
